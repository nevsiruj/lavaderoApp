<template>
  ...
  <!--, query: { isAdmin: true } }"> -->
  <router-link :to="{ path: '/formTipoServicio' }" class="btn btn-base btn-success flex items-center space-x-2">
    <i class="fas fa-plus-circle"></i>
    <span>Agregar tipo de servicio</span>
  </router-link>
  <router-link :to="{ path: '/tipoServicios' }" class="btn btn-base btn-primary flex items-center space-x-2">
      ...
      <tr v-for="tipoServicio in tipoServicios" :key="tipoServicio.id">
        ...
        <td class="px-6 py-3 whitespace-nowrap text-lg">
          <div class="flex space-x-4">
            <button class="text-blue-600 hover:text-blue-800 focus:outline-none text-xl" @click="editarTipoServicio(tipoServicio)">
              <i class="fas fa-edit"></i>
            </button>
            <button class="text-red-600 hover:text-red-800 focus:outline-none text-xl" @click="deleteTipoServicio(tipoServicio.id)">

<template>
  <div class="viewport">
    <div class="card w-max m-auto p-4">
      <div class="mb-3">
        <router-link :to="'owner/usuarios'" class="text-emerald-300 hover:text-emerald-600">&lt; Volver atrás
        </router-link>
      </div>
      <h1 class="font-bold">Usuarios</h1>
      <form @submit.prevent="submitForm">
        <div class="form-group">
          <label for="nombre" class="text-black">User</label>
          <input type="text"
        ...
        <div class="form-group">
          <label for="apellido" class="text-black">Empresa</label>
          <input type="text"
            class="form-input mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-300 focus:ring focus:ring-blue-200"
            id="apellido" v-model="form.apellido" />
        </div>
        <div class="form-group">
          <label for="dni" class="text-black">Contraseña</label>
          <input type="password"
            class="form-input mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-300 focus:ring focus:ring-blue-200"
            id="dni" v-model="form.dni" @input="validatePassword" />
        </div>
        <div class="form-group">
          <label for="username" class="text-black">Confirmar contraseña</label>
          <input type="password"
            class="form-input mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-300 focus:ring focus:ring-blue-200"
            id="username" v-model="form.username" />
        </div>
        <div v-if="passwordError" class="text-red-600 mt-2">{{ passwordError }}</div>
        <div class="text-gray-500 mt-2">La contraseña debe contener al menos una mayúscula, un punto y un número.</div>

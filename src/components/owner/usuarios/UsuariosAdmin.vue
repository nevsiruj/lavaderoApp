    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
      <table class="min-w-full divide-y divide-gray-200 mt-4">
        <thead class="bg-emerald-300">
          <tr>
            <th class="px-4 py-2 text-xs text-gray-500 uppercase tracking-wider whitespace-nowrap">User</th>
            <th class="px-4 py-2 text-xs text-gray-500 uppercase tracking-wider whitespace-nowrap">Empresa</th>
        ...
        <tbody class="bg-white divide-y divide-gray-200 text-left">
          <tr v-for="usuario in usuarios" :key="usuario.id">
            ...
            <td class="px-6 py-4 lg:text-left">
              <a @click="editarUsuario(usuario)" href="#" class="font-medium text-gray-800 dark:text-white hover:underline">
              ...
              <a @click="deleteModal(usuario.id)" href="#" class="font-medium text-red-500 dark:text-white hover:underline lg:ml-5">
                <svg class="w-4 h-4 inline-block text-gray-800 dark:text-white" aria-hidden="true"
                  xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                  <path stroke="#ef4444" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
  ...
  setup() {
    ...
    const editarUsuario = (user) => {
      router.push({
        ...
        query: { isAdmin: false, id: user.id },
      });
    };

    const deleteUsuario = async (usuarioId) => {
      try {
        await userService.removeUsuario(usuarioId);
      } catch (error) {
        console.error(error);
      }
      fetchUsuarios();
    ...
    const deleteModal = (usuarioId) => {
      if (modalComponent.value) {
        modalComponent.value.getModal(usuarioId);
      } else {
        console.error("Modal component is not mounted or available.");
      }
    };
